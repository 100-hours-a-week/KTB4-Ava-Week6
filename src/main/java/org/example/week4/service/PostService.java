...
import org.example.week4.domain.*;
...
import org.example.week4.repository.*;
...
public class PostService {
    ...
    public PostResponseDto createPost(@Positive Long userId, PostRequestDto request, MultipartFile image) {
        ...
        Long fileId = file == null ? null : file.getId();

        Post post = new Post(null, userId, request.getTitle(), request.getContent(), fileId);

        Post savedPost = postRepository.save(post);

        String fileUrl = fileId == null ? null : fileService.getFileUrl(fileId);

        return new PostResponseDto(savedPost, user, fileUrl);
    }
    ...
    public PostResponseDto getPost(Long postId) {
        ...
        User user = userRepository.findById(post.getUserId()).orElseThrow(() -> new NotFoundException("user_not_found"));
        ...
        if (post.getFileId() == null) {
            ...
            File file = fileRepository.findById(post.getFileId()).orElseThrow(() -> new NotFoundException("file_not_found"));
            String fileUrl = fileService.getFileUrl(file.getId());
            return new PostResponseDto(post, user, fileUrl);
        }
    }
    ...
    public PostResponseDto updatePost(@Positive Long userId, @Positive Long postId, @Valid PostUpdateRequestDto request, MultipartFile image) {
        ...
        if (hasImage) {
            File file = fileService.storeFile(image, userId, FileCategory.POST_ATTACHMENT);
            post.updateFileId(file.getId());
        }

        String fileUrl = post.getFileId() == null ? null : fileService.getFileUrl(post.getFileId());
    ...
    public PostResponseDto likePost(@Positive Long userId, @Positive Long postId) {
        ...
        likeRepository.findByPostIdAndUserId(postId, userId)
                .ifPresentOrElse(
                        ...
                        () -> {
                            likeRepository.save(new Like(null, postId, userId));
    ...
    private PostResponseDto toPostResponseDto(Post post) {
        ...
        if (post.getFileId() != null) {
            imageUrl = fileService.getFileUrl(post.getFileId());
        }

        return new PostResponseDto(post, user, imageUrl);
    }
}
